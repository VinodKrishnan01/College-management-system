...
const Project = require("../../models/Other/project.model.js");
...
const getStudentsBySkill = async (req, res) => {
    try {
        ...
        for (const project of projects) {
            const [firstName, lastName] = project.student.split(' ');
            
            if (!studentData[project.student]) {
                const student = await studentDetails.findOne({ firstName, lastName });
                if (student) {
                    studentData[project.student] = {
                        studentInfo: student,
                        projects: []
                    };
                }
            ...
            if (studentData[project.student]) {
                studentData[project.student].projects.push({
                    title: project.title,
        ...
        res.json({
            success: true,
            message: `Found ${formattedData.length} students with ${skill} skills`,
...
module.exports = { getDetails, addDetails, updateDetails, deleteDetails, getCount, getStudentsBySkill }